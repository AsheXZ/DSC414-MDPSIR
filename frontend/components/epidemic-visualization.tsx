...
const STATE_COLOR: Record<"S" | "I" | "R", string> = {
  S: "#22c55e",
  I: "#ef4444",
  R: "#38bdf8",
};
...
function discretize(
  ...
  N: number,
  budgetCap: number,
  bins: { s: number; i: number; r: number; b: number },
) {
...
function greedyActionFromQ(
  ...
  idx: { s: number; i: number; r: number; b: number },
): number {
  const table = qTableValues as unknown[];
  const qValues = safeQValues(
    (table?.[idx.s] as unknown[] | undefined)?.[idx.i] &&
      ((table[idx.s] as unknown[])[idx.i] as unknown[] | undefined)?.[idx.r] &&
      ((((table[idx.s] as unknown[])[idx.i] as unknown[])[idx.r] as unknown[] | undefined)?.[idx.b]),
  );
...
function buildTimeline(
  ...
  policy: (ctx: { S: number; I: number; R: number; budget: number; N: number; t: number }) => number,
  horizon = HORIZON,
): TimelinePoint[] {
  const N = Math.max(1, params.population);
  let S = Math.max(0, params.population - params.initialInfected - params.initialRecovered);
  ...
  for (let t = 1; t <= horizon; t += 1) {
    let action = policy({ S, I, R, budget, N, t });
    ...
    const newInfections = betaEff * (SAfterVax * I / N);
...
function cellClass(state: "S" | "I" | "R") {
...
export function EpidemicVisualization({
  ...
  const qTableTimeline = useMemo(() => {
    ...
    return buildTimeline(params, ({ S, I, R, budget, N }) => {
      const idx = discretize(S, I, R, budget, N, params.budget, bins);
      return greedyActionFromQ(qTablePayload.values, idx);
    });
  }, [params, qTablePayload]);

  const availableModes: VisualizationMode[] = ["simulation"];
  if (artifactTimeline.length > 0) availableModes.push("artifact");
  if (qTableTimeline.length > 0) availableModes.push("qtable");
  ...
  const population = Math.max(1, current.S + current.I + current.R);
  ...
  const spreadNetwork = useMemo(() => {
    ...
    const rCount = Math.max(0, totalNodes - sCount - iCount);

    const nodes: Array<{ id: number; x: number; y: number; state: "S" | "I" | "R" }> = [];
    ...
    return { nodes, links };
  }, [current.I, current.S, current.t, population]);
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-app-muted">
              Source mode: {mode === "artifact" ? "Trained episode replay" : mode === "qtable" ? "Q-table greedy policy" : "Browser simulation"}
            </p>
          </div>
